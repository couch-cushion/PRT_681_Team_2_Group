      ...
      <section className="workspace">
        ...
        <div className="task-list">
          ...
          {loading ? <div className="empty-state">Loading your tasks...</div> : tasks.length === 0 ? (
            <div className="empty-state">Add your first task to get started. Delete appears on the right side of each task.</div>
          ) : (
            <div className="tasks">
              {tasks.map((task) => (
                <article className={`task-item ${task.isCompleted ? 'completed' : ''}`} key={task.id}>
                  ...
                  <button className="delete-button" onClick={() => deleteTask(task.id)} aria-label={`Delete ${task.title}`} title="Delete task">×</button>
                </article>
              ))}
            </div>
          )}
